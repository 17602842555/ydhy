...
- Added `子公司监管 AI 周报` to the third-level supervision page. The panel reuses `/api/ai/insights`, the Ark Coding Plan configuration, backend task-calendar supervision data, saved-cache behavior, and a preset weekly-report prompt with a copyable report draft.
